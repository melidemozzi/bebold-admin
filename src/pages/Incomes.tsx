...
import { Search, PlusCircle, Trash2, Pencil, ChevronLeft, ChevronRight, Upload, RefreshCw } from 'lucide-react';
...
export default function Incomes() {
  ...
  function traerClientesMes() {
    ...
    if (activosConMonto.length === 0) { alert('No hay clientes activos con monto definido.'); return; }
    ...
    if (nuevos.length === 0) { alert('Todos los clientes activos ya están cargados este mes.'); return; }
    setIncomes(prev => [...prev, ...nuevos as Income[]]);
  }

  const handleImport = (parsedData: any[], period: string) => {
    if (parsedData.length === 0) {
      alert('No se encontraron registros. Verificá que las columnas estén bien seleccionadas.');
      return;
    }
